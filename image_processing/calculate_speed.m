function [vel_anterior_sign, vel_posterior_sign] = calculate_speed()
...
switch list{indx}
    ...
    case 'Calcium'
        ...
        if str2double(filename)==0
            ...
            dims = [1 35];
            fnm = size(imfinfo(filename),1);
            definput = {'63','180','1800','1', num2str(fnm),'2'};
...
%                 end
                ...
                posnum = size(xyz,1);
                if ~isequal(posnum,fnm)
                    fprintf(['Number of position data: ' num2str(posnum) '\n']);
                    fprintf(['Number of entire recording frames: ' num2str(fnm) '\n']);
                    fprintf('Stage position info from the loaded file is not complete.\n')
                else
                    % xyz is a cell array
                    % Not sure if using cellfun is faster here
                    pos_stage = zeros(size(xyz,1),2);
                    for jj = 1:size(xyz,1)
                        for ii = 1:2
                            pos_stage(jj,ii) = str2double(xyz{jj,ii});
                        end
                    end
                    ...
                    if str2double(position_file)==0
                        ...
                        if size(position_file, 2) == 2
                            ...
                            pos_anterior = neuron_position_data(istart:iend,:);
                            signal_anterior = signal{1,1}./signal_mirror{1,1};
                            signal_anterior = signal_anterior(istart+1:iend); % To match the number of data points in velocity
                            ymax_1 = max(signal_anterior);
                            load(position_file{2});
                            pos_posterior = neuron_position_data(istart:iend,:);
                            signal_posterior = signal{1,1}./signal_mirror{1,1};
                            signal_posterior = signal_posterior(istart+1:iend); % To match the number of data points in velocity
                        ...
                        plot([1 length(vel_ap_sign_smd_mean)], [0 0], 'k', 'linewidth', 1);
...
%                 'Minimum consecutive frames'};
            ...
            dims = [1 35];
            fnm = size(imfinfo(filename),1);
            definput = {'4','75','2000','1',num2str(fnm)};
            answer = inputdlg(prompt,dlgtitle,dims,definput);
...
%                 end
                ...
                if ~isequal(posnum,fnm)
                    ...
                    if str2double(position_file)==0
                        ...
                        load(position_file, 'centerline_data_spline');
                        % Head position
