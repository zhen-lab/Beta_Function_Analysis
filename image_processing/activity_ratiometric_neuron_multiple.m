...
for nm = 1:nummov
    ...
    range_all{nm} = range;
    framestruct_all = framestruct;
    
    % Clear other variables to enable loading for the next recording
    clearvars -except imagelist_all imagelist_g_all imagelist_r_all filename_all range_all framestruct_all nummov
...
for nm = 1:nummov
    ...
    if updated~=1
        ...
        movpara.th = str2double(answer{1,1});
        movpara.gsfilt = str2double(answer{2,1});

%         % Find proper adaptive threshold
%         movpara.movpara.th = .25;
%         % Apply threshold and filtering to all numerator images
%         movpara.movpara.gsfilt = .6; 
    ...
    %         imagelist_segratiofilt{ni,1} = imgaussfilt(imagelist_segratio{ni,1},movpara.movpara.movpara.gsfilt);
    ...
    th_all{nm,1} = movpara.th;
    gsfilt_all{nm,1} = movpara.gsfilt;
...
for nm = 1:nummov
    ...
    definput = {'2','0'};
    answer = inputdlg(prompt,dlgtitle,dims,definput);
    movpara.maxi = str2double(answer{1,1});
    movpara.mini = str2double(answer{2,1});

    clrtheme = 'inferno';
%     movpara.mini = 0; 
%     movpara.maxi = 3; 
    subplot(121); imagesc(imagelist_g{end,1}); caxis([0 3000]); title('GCaMP'); axis equal; colormap(clrtheme);
    % subplot(132); imagesc(imagelist_segratio{end,1}); caxis([movpara.movpara.mini movpara.movpara.maxi]); title('Segmented ratio'); axis equal; colormap(clrtheme);
    subplot(122); imagesc(imagelist_segratiofilt{end,1}); caxis([movpara.mini movpara.maxi]); title('Segmented filtered ratio'); axis equal; colormap(clrtheme);
    
    mini_all{nm,1} = movpara.mini; 
    maxi_all{nm,1} = movpara.maxi; 
...
for nm = 1:nummov
    ...
    if ~exist('movpara','var')
        ...
        % Data should include gfp, np, rfp in matrix format
        ...
        if size(np,2)/2==1
            ...
            prompt = {...
                ...
                ['Movie name for ' filename_all{nm,1}]};
        %         'Lower bound of activity:',...
        %         'Upper bound of activity:'};
            inputdlgtitle = 'Parameters for GIF';
            dims = [1 35];
            definput = {'3',num2str(size(imagelist_g,1)),...
                '0.3','1','0',...
                'DD','5',num2str(7*ones(1,neurontotal)),...
                '',...
...
%             movname = inputdlg(['Move name for ' filename]);
        ...
        %         plot([headbody headbody], [movpara.movpara.mini movpara.movpara.maxi], ':k');
        % %         [~,hobj,~,~] = legend([bd bv], {'Dorsal', 'Ventral'}); legend('boxoff');
        % %         hl = findobj(hobj,'type','line'); set(hl,'LineWidth',0.5);
        %         set(gca, 'ticklength', [0 0]); ylim([movpara.movpara.mini movpara.movpara.maxi]); 
        %         xlabel('Body segment'); ylabel('GCaMP/RFP'); title('Muscle activity along anterior-posterior axis');
                ...
                if frm == 1 && nm ==1
                  imwrite(imind, cm, [fullfilename '_regsegfiltrat.gif'], 'gif', 'LoopCount', inf, 'DelayTime', movpara.ff/fps);
                else
                  imwrite(imind, cm, [fullfilename '_regsegfiltrat.gif'], 'gif', 'WriteMode', 'append', 'DelayTime', movpara.ff/fps);
                end
        
            end
        
            save([fullfilename '_movpara.mat'], 'tform', 'thetaNeuronRadAll', ....
                'movpara');
