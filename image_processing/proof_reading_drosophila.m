function varargout = proof_reading_drosophila(varargin)
...
% Last Modified by GUIDE v2.5 22-Aug-2014 20:28:57
...
hold on; % Draw a circle for the mirror of ROI in RFP screen
...
axes(handles.Image_axes);
hold on;  handles.points{idx}=rectangle('Curvature', [1 1],'Position',[x-handles.r(idx), y-handles.r(idx), 2*handles.r(idx), 2*handles.r(idx)],'EdgeColor', 'g');
set(handles.points{idx},'ButtonDownFcn', 'proof_reading(''ButtonDownPoint_Callback'',gcbo,[],guidata(gcbo))');
...
if ~isempty(handles.neuronal_position{handles.frame_number, 1})
    ...
    %         [GC(j), handles.F_sorted, handles.F_thresh] = calculate_intensity(img_bw);
            handles.neuronal_position_left{j, k}(1) = x;
    ...
    %         GC_m(j) = calculate_intensity(img_bw_m);
        ...
        plot(((1:handles.frame_number)+handles.istart-1), smooth(handles.signal{k},1), 'g'); hold on;
        plot(((1:handles.frame_number)+handles.istart-1), smooth(handles.signal_mirror{k},1), 'r');
        
        subplot(handles.neuron_number * 2, 1, 2 * k); 
        plot(((1:handles.frame_number)+handles.istart-1), smooth(handles.signal_GFPoverRFP{k},1), 'k'); hold on;
        plot(((1:handles.frame_number)+handles.istart-1), smooth(GC_GoR_mean(:, k),1), 'k:');
