...
if length(options.is_bad_frame) == 1
    options.is_bad_frame = repmat(options.is_bad_frame, [size_T, 1]);
elseif length(options.is_bad_frame) ~= size_T
    error('length(is_bad_frame) is incorrect');
end
...
feature = struct(...
    ...
    'is_bad_frame', options.is_bad_frame, ...
