...
if exist(path, 'dir')
    warning('Path %s exists. Exiting.', path);
    return
end
make_directory(path);
...
if ~isfield(options, 'Deflate'); options.Deflate = 5; end;
...
index = 1;
% main data loop
for timeId = 1:size(I, 4)
    timeId2 = options.t(1) + timeId - 1;
    disp(sprintf('%05d / %05d \n', timeId, size(I,4)));
    for colId = 1:colors
        for levelId = 1:noLevels
            ...
            h5create(h5_filename, datasetName, [newH, newW, newZ], ...
                'Datatype', 'int16', 'ChunkSize', ChunkSize, 'Deflate', options.Deflate);
