function scatterbar3(X,Y,Z,width,DorV)
...
for j=1:r,
    for k=1:c,
        if ~isnan(Z(j,k))
            drawbar(X(j,k),Y(j,k),Z(j,k),width/2, DorV)
        end
    end
end
...
function drawbar(x,y,z,width,DorV)
base = 100; 
if DorV==1, bclr = [0 1 0]; 
else bclr = [1 0 0.9];
end
h(1)=patch([-width -width width width]+x,[-width width width -width]+y,base+[ 0 0 0 0],bclr, 'edgecolor', 'none');
h(2)=patch(width.*[-1 -1 1 1]+x,width.*[-1 -1 -1 -1]+y,base+z.*[0 1 1 0],bclr,'edgecolor', 'none');
h(3)=patch(width.*[-1 -1 -1 -1]+x,width.*[-1 -1 1 1]+y,base+z.*[0 1 1 0],bclr,'edgecolor', 'none');
h(4)=patch([-width -width width width]+x,[-width width width -width]+y,base+[z z z z],bclr,'edgecolor', 'none');
h(5)=patch(width.*[-1 -1 1 1]+x,width.*[1 1 1 1]+y,base+z.*[0 1 1 0],bclr,'edgecolor', 'none');
h(6)=patch(width.*[1 1 1 1]+x,width.*[-1 -1 1 1]+y,base+z.*[0 1 1 0],bclr,'edgecolor', 'none');
