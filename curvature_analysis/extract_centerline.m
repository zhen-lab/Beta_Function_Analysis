...
%     rb = imagelist{2*j-1+2*(istart-1), 1}; % GFP channel
%     c = imagelist{2*j+2*(istart-1), 1}; % RFP channel
...
%     adj = imcomplement(adj);
    ...
    if j == 1
        ...
        figure (1);
        imagesc(adj); % Adjust low contrast gray image
...
%         bkg_c=mean(mean(c(cropy1_bkg:cropy2_bkg,cropx1_bkg:cropx2_bkg))); 
        ...
        text(10,60,'Click on vulva', 'Color', 'white');     
    ...
    imagesc(adj); axis equal; axis off; hold on;
...
%     adj_rb_c = wiener2(adj); 
    adj_rb_c = imclearborder(adj);
...
%     bw = closeBW_edge;
    ...
    % Plot ventral and dorsal edges, and skeleton
    plot(path1(:,2), path1(:,1), '-b', 'LineWidth', 1); hold on;
    plot(path2(:,2), path2(:,1), '-r', 'LineWidth', 1); hold on;
    plot(midline_mixed(:,2), midline_mixed(:,1), '-w', 'LineWidth', 2);
...
%     figure (1); hold on; plot(line(:,1), line(:,2), 'w.', 'linewidth', 2);
    ...
    centerline_data(:, 2*j-1) = line(:, 1); 
    centerline_data(:, 2*j) = line(:, 2);
...
%end of calculations for 1 frame
